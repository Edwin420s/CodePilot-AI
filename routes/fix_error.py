...
async def fix_error(
    ...
    system_prompt = "You are a debugging expert. Analyze this error and suggest fixes."
    ...
    try:
        ...
        response_text = "⚠️ AI service is temporarily unavailable. Please try again later."

    add_message(session_id, "assistant", response_text)

    elapsed = time.time() - start
    logger.info(f"POST /fix-error | session={session_id} | time={elapsed:.2f}s")
    return {"response": response_text, "session_id": session_id}

@router.post("/fix-error/stream")
@limiter.limit("10/minute")
async def fix_error_stream(
    ...
    async def wrapper():
        ...
        logger.info(f"Fix error stream completed for session {session_id}")
